...
DOCS_PREAMBLE = (
    "请先检索公司技术资料库再作答（不要凭常识直接猜选项）：\n"
    "先输出 <search>关键词</search>（系统会在资料库查并把片段以「[检索结果]」回灌给你）。\n"
    "检索词只保留核心术语；通常一次定向检索后就应作答，仅当首次没有相关资料时才换关键词再查一次。\n"
    "拿到资料后，按题目要求作答，并把答案放入 \\boxed{...}（单选/多选题填选项字母，如 \\boxed{B} 或 \\boxed{A,C}）。"
    "仅当检索明确未命中相关资料时，才可根据题面直接作答。\n\n"
    "下面是一个完整示例（仅演示交互格式）：\n"
