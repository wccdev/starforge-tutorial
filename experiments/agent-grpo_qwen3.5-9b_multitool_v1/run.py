#!/usr/bin/env python
# 多轮多工具 Agent（检索 + 计算 + 代码执行）GRPO 训练脚本（NeMo-RL 0.7.0）。
# 改编自官方 examples/run_grpo_sliding_puzzle.py：环境换成自定义多工具环境，
# 数据集随机生成三类需要工具的任务。由 grpo recipe 的显式入口调用；样板流程见 common/bootstrap.py。
import itertools
import os
import random
import sys
...
def main():
    config, _ = bootstrap.load_experiment_config(
        THIS_DIR, MasterConfig, description="多轮多工具 GRPO 训练"
    )
    ...
    bootstrap.run_grpo(config, tokenizer, dataset, val_dataset, {TASK_NAME: env})
