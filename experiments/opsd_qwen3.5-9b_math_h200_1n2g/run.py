...
#
# 注意：distillation 的 setup()/train() 与 GRPO 不同构（11 元组、双 policy），
#       故训练循环留在本文件，只有配置/运行时样板走 common/bootstrap.py。
...
def main() -> None:
    # opsd 段是本仓库自定义的（不属于 MasterConfig schema），构造前先取出。
    config, extras = bootstrap.load_experiment_config(
        THIS_DIR, MasterConfig, description="OPSD 自蒸馏训练", pop_sections=("opsd",)
    )
    opsd_cfg: dict[str, Any] = extras["opsd"]
    tokenizer = bootstrap.init_runtime(config, "distillation")

    # ── 数据 ────────────────────────────────────────────────────────────────
    # 隐私/大体量数据提前放到集群共享盘，在 config.data.data_dir 写绝对路径。
    # OPSD_DATA_DIR 仅作可选覆盖；不能优先于 config，否则 launcher 的
    # 仓库内默认会把路径拐进 working_dir（jsonl 被 gitignore，随包没有文件）。
    data_cfg: dict[str, Any] = config.data
    data_dir = bootstrap.resolve_data_dir(
        data_cfg, "OPSD_DATA_DIR",
        "请在 config.yaml 的 data.data_dir 写集群绝对路径"
