...
改造前：每个实验的 run.py 自己 `from common.algorithms.opsd import install_opsd` 再调用。
后果不是「monkey-patch 不好」——对装不了新依赖的内网集群，零依赖补丁是决定性优势——
而是**平台看不见它**：无法知道哪个作业用了哪个补丁的哪个版本，无法复用到别的实验，
也无法在提交时校验「这个方法要求的补丁在不在包里」。

现在补丁由 recipe 的 `plugins:` 声明，平台记账、launcher 统一装载。手段没变，
它成了一等概念。
