"""训练侧可观测性采集：主动把训练指标 + 每卡硬件上报到 StarForge Console。

启用方式（由 console 提交作业时注入环境变量，本地直跑则全程 no-op）：
  STARFORGE_ENDPOINT / STARFORGE_RUN_ID / STARFORGE_TOKEN

NeMoRLAdapter 的运行时 bridge 先 apply_patch() 给 nemo_rl.utils.logger.Logger
挂上 StarForgeLogger 后端，再运行原始训练入口，无需改 NeMo-RL 源码。
...
同一套传输设施，但不依赖 nemo_rl：
    ...
    # HF Trainer / TRL：callbacks=[report.StarForgeCallback()]
"""
